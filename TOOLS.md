...
| `get_homebridge_product_media` | Returns bounded Shopify media resources, a native inline image preview, an exact Markdown image fallback, and a ChatGPT-compatible read-only product card; an exact SKU resolves its variant ID and prioritizes Shopify-associated media. | None |
